...
{
    //settings
    public string RaycastTargetGOTag = "Untagged";
    //fx
    ...
    void Update()
    {
        ...
        if (Input.GetMouseButtonDown(0))
        {
            ...
            {
                if (hit.transform.CompareTag(RaycastTargetGOTag))
                {
    ...
    {
        #region sfx
        Transform clone = Instantiate(explodePrefab, target.position, Quaternion.identity) as Transform;
    ...
    void CreateMeshPiece(float extrudeSize, Vector3 pos, Material mat, int index, Vector3 faceNormal, Vector3 v1, Vector3 v2, Vector3 v3, Vector2 uv1, Vector2 uv2, Vector2 uv3)
    {
        ...
        go.tag = RaycastTargetGOTag; // set this only if should be able to explode this piece also
